...
export async function getHotels({
  ...
  try {
    ...
    if (hotels.length > 0) return { hotels, source };
  } catch {
    // SerpAPI sin cuota o fallo → usamos respaldo OpenStreetMap.
  }

  const fallback = await getHotelsFromOverpass({ lat, lon });
  if (fallback.length > 0) return { hotels: fallback, source: "OpenStreetMap" };
  return { hotels: [], source: "OpenStreetMap" };
}
