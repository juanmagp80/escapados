...
import { candidateDestinations, DESTINATIONS } from "@/lib/destinations/catalog";
...
export async function runSearch({
  ...
  if (transport === "plane" && fromAirport) {
    ...
    if (dynamic.length > 0) {
      ...
      // En modo vacaciones, también incluimos destinos del catálogo con aeropuerto
      // que no aparecen en la lista dinámica, para buscar más ciudades,
      // especialmente españolas y europeas.
      if (vacations) {
        for (const cat of DESTINATIONS) {
          if (cat.airport && cat.airport !== fromAirport && !added.has(cat.slug)) {
            merged.push({ ...cat, priceHint: null, image: cat.image });
            added.add(cat.slug);
          }
        }
      }
  ...
  const destinations = (
    await Promise.all(
      names.map(async (d) => {
        ...
        if (transport === "car") {
          ...
          if (canFly && (wholeMonth || vacations) && startDate) {
            ...
            if (monthFlights.length > 0) {
              ...
              // En modo vacaciones, no descartamos destinos sin vuelos: los
              // incluimos con el priceHint para que aparezcan en los resultados.
              dest.flightOptions = [];
              dest.noFlights = true;
              dest.transportCost = d.priceHint
                ? d.priceHint * (Number(travelers) || 2)
                : null;
              dest.transportLabel = "✈️ Sin vuelos en este período";
            } else {
              return null;
            }
        ...
        // Destinos sin vuelos en modo vacaciones: no hay precio real, los
        // señalamos con estimatedCost=null para ordenarlos al final.
        if (dest.noFlights) {
