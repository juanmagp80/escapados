...
import { formatKm, formatDuration, localIso, addDaysIso } from "@/lib/utils/format";
...
function weekendPairsInMonth(startDate) {
  ...
  for (let day = 1; day + 3 <= last; day++) {
    ...
    if (dow === 4) {
      ...
      pairs.push({ outbound: isoBase, returnDate: addDays(base, 2) }); // vie→dom
      pairs.push({ outbound: isoBase, returnDate: addDays(base, 3) }); // vie→lun
    } else if (dow === 6) {
      pairs.push({ outbound: isoBase, returnDate: addDays(base, 2) }); // sáb→lun
    }
  }
  return pairs;
}
...
export async function runSearch({
  ...
  const names = candidateDestinations(origin, {
    ...
    await Promise.all(
      names.map(async (d) => {
        ...
        if (transport === "car") {
          ...
          if (canFly && wholeMonth && startDate) {
            // Busca los fines de semana del mes y se queda con todas las opciones.
            const pairs = weekendPairsInMonth(startDate);
            const monthFlights = [];
            ...
            if (monthFlights.length > 0) {
              ...
              dest.transportLabel = `✈️ ${monthFlights[0].airline || "Vuelo"} (mes)`;
            } else {
              return null;
            }
