  ...
  "days": [
    {
      ...
      "title": "string",
      "activities": [
        { "time": "10:00", "name": "string", "description": "string", "duration": "2h", "lat": 37.1773, "lon": -3.5986 }
      ],
      "restaurants": [ "string" ]
    }
  ]
...
// Chat conversacional para refinar el itinerario: el usuario puede pedir
// cambios ("prefiero algo más tranquilo", "quita el tercer día y añade un
// balneario") y Gemini devuelve un itinerario actualizado.
export function buildRefinePrompt({ destination, currentItinerary, userMessage }) {
  return `Eres un planificador de escapadas para parejas. El usuario quiere modificar su itinerario.
...
REGLAS ESTRICTAS (crítico):
- Mantén el mismo formato JSON que el itinerario actual.
- NO inventes precios, horarios de apertura, disponibilidad ni datos económicos.
- NO inventes lugares que no aparezcan en el itinerario actual. Si necesitas sugerir algo genérico, márcalo como "sugerido".
- Adapta las actividades a la petición del usuario.
- Mantén el número de días del itinerario actual salvo que el usuario pida cambiarlo.
- Responde SOLO con JSON válido, sin texto adicional, sin markdown.
...
export async function refineItinerary({ destination, currentItinerary, userMessage }) {
  const prompt = buildRefinePrompt({ destination, currentItinerary, userMessage });
