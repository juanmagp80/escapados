...
// Límite de combinaciones a buscar en el fallback de searchFlightOptions,
// para no disparar miles de llamadas a la API cuando Travelpayouts falla.
// Con 2-5 noches, ~40 pares cubren una ventana de ~12 días de comienzos.
const FALLBACK_MAX_PAIRS = 40;
...
export async function searchFlightOptions({
  ...
  const results = [];
  const minN = minNights || 2;
  const maxN = (maxNights && minNights) ? maxNights : 5;
  const lastDay = endDate || addDaysIso(startDate, 7);
  // Busca combinaciones ida+vuelta de 2-5 noches desde varios días de comienzo
  // dentro del período de vacaciones (no solo desde startDate), para encontrar
  // las combinaciones más económicas posibles.
  let outbound = startDate;
  let searched = 0;
  while (outbound <= addDaysIso(lastDay, -minN) && searched < FALLBACK_MAX_PAIRS) {
    for (let n = minN; n <= maxN; n++) {
      const returnDate = addDaysIso(outbound, n);
      if (returnDate > lastDay) break;
      searched++;
    ...
    outbound = addDaysIso(outbound, 1);
  }
